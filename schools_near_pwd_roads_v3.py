#!/usr/bin/env python3
"""
Find schools adjacent to Kerala PWD roads - v3 (post-review corrected coordinates)
====================================================================
Same GIS logic as schools_near_pwd_roads_v2.py (point-to-segment distance
against malappuram_pwd_roads_v2.geojson, the Kerala PWD RMMS road network),
but run against schools_malappuram_my_maps_import_v2.csv - the coordinate
DB after applying the manual review corrections captured via
schools_road_adjacent_maps_check.html and merged in by
apply_road_review_corrections.py.

Also loads the v2 results (schools_all_road_distances_v2.csv, pre-review
coordinates) so it can print which specific schools changed adjacency
status purely because of the coordinate corrections - isolated from the
earlier v1(OSM)-vs-v2(RMMS) road-data change.

Usage:
    python3 apply_road_review_corrections.py   # produces the v2 coordinate CSV first
    python3 schools_near_pwd_roads_v3.py

Output (written next to this script):
    schools_all_road_distances_v3.csv
    schools_adjacent_to_pwd_roads_v3.csv
    schools_adjacent_to_pwd_roads_v3.pdf
"""
...
SCHOOLS_CSV = "schools_malappuram_my_maps_import_v2.csv"
ROADS_GEOJSON = "malappuram_pwd_roads_v2.geojson"
PRIOR_DISTANCES_CSV = "schools_all_road_distances_v2.csv"  # pre-review coordinates, for comparison only
ADJACENCY_THRESHOLD_M = 100

ALL_OUT = "schools_all_road_distances_v3.csv"
ADJACENT_CSV_OUT = "schools_adjacent_to_pwd_roads_v3.csv"
ADJACENT_PDF_OUT = "schools_adjacent_to_pwd_roads_v3.pdf"
...
def load_schools(path):
    if not os.path.exists(path):
        print(f"[!] {path} not found in this folder. Run apply_road_review_corrections.py first.")
        sys.exit(1)
    schools = []
    skipped_no_coords = 0
...
def load_prior_status(path, threshold):
    """Load pre-review (v2) per-school distances, keyed by school name, for the
...
def main():
    print("=" * 70)
    print("Finding schools adjacent to Kerala PWD roads v3 (post-review coordinates)")
    ...
    print(f"\n{len(adjacent)} of {len(results)} checked schools are within "
          f"{ADJACENCY_THRESHOLD_M}m of a PWD road (v3 / post-review)")
    ...
    # --- v2 (pre-review) vs v3 (post-review) comparison ---
    print("\n" + "=" * 70)
    print("COMPARISON: v2 (pre-review coordinates) vs v3 (post-review coordinates)")
    print("=" * 70)
    ...
    if prior_status is None:
        ...
        v3_status = {r['name']: r['nearest_distance_m'] <= ADJACENCY_THRESHOLD_M for r in results}
        newly_flagged = []
        no_longer_flagged = []
        for name, was_adjacent in prior_status.items():
            is_adjacent = v3_status.get(name)
            if is_adjacent is None:
                continue  # school wasn't in the v3 result set (shouldn't normally happen)
        ...
        print(f"v2 (pre-review):  {old_count} of {len(prior_status)} schools within {ADJACENCY_THRESHOLD_M}m")
        print(f"v3 (post-review): {new_count} of {len(results)} schools within {ADJACENCY_THRESHOLD_M}m")
        print(f"\nNewly flagged as adjacent in v3 ({len(newly_flagged)} schools):")
        for name in sorted(newly_flagged):
            print(f"  + {name}")
        print(f"\nNo longer flagged as adjacent in v3 ({len(no_longer_flagged)} schools):")
...
def save_pdf(adjacent):
    ...
    total = len(adjacent)
    story.append(Paragraph("Schools Adjacent to PWD State Highways / MDRs (v3 - post-review)", title_style))
