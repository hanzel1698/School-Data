...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .sync-row { display: flex; align-items: center; justify-content: space-between; gap: 8px;
    margin: 0 12px 10px; font-size: 11px; color: var(--muted); }
  .sync-row button.link { color: var(--accent); background: none; border: none; padding: 0;
    font-size: 11px; cursor: pointer; text-decoration: underline; flex: 0 0 auto; }
...
<div id="syncPanel" class="key-panel">
  <div class="key-panel-title">🔄 GitHub sync</div>
  <div class="key-panel-body">
    Paste a GitHub <b>fine-grained personal access token</b>, scoped to <b>only this repo</b>
    with <b>Contents: Read and write</b> permission — nothing else. It's saved only in this
    browser, and used to save your review progress to <code>road_review_state.json</code> in
    the repo so it can be loaded back on any device.
    <a href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noopener">Create a token</a>.
  </div>
  <div class="key-panel-row">
    <input id="ghTokenInput" type="password" placeholder="github_pat_…" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false">
    <button id="toggleGhTokenVis" type="button" title="Show/hide">👁</button>
  </div>
  <button id="saveGhTokenBtn" class="action blue" style="width:100%">Save &amp; Sync</button>
  <button id="clearGhTokenBtn" class="action ghost" style="width:100%;margin-top:6px">Disable Sync</button>
  <div id="ghTokenError" class="key-error"></div>
</div>

<div class="sync-row">
  <span id="syncStatus" class="sync-status">Checking for synced state…</span>
  <button id="syncSetupBtn" class="link" type="button">set up</button>
</div>
...
const GH_OWNER = "hanzel1698";
const GH_REPO = "School-Data";
const GH_BRANCH = "main";
const GH_PATH = "road_review_state.json";
const GH_TOKEN_STORAGE = "roadReviewGithubToken_v1";
const GH_SHA_STORAGE = "roadReviewGithubFileSha_v1";
...
document.getElementById("exportJsonBtn").addEventListener("click", () => {
  ...
  if (ghToken()) pushReviewsToGitHub();
});
...
function updateSyncPanelVisibility() {
  document.getElementById("syncSetupBtn").textContent = ghToken() ? "manage" : "set up";
}
...
function scheduleGithubPush() {
  ...
  ghPushTimer = setTimeout(() => pushReviewsToGitHub(), 2000);
}

async function pushReviewsToGitHub(isRetry) {
  const token = ghToken();
  if (!token) return;
  setSyncStatus("Syncing…");
  const apiUrl = `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${GH_PATH}`;
  let sha = localStorage.getItem(GH_SHA_STORAGE) || null;
  try {
    if (!sha) {
    ...
    const body = {
      message: "Update road review state (" + new Date().toISOString() + ")",
      content: utf8ToBase64(JSON.stringify(reviews, null, 2)),
      branch: GH_BRANCH
    };
    if (sha) body.sha = sha;
    ...
    if (putRes.ok) {
      ...
      localStorage.setItem(GH_SHA_STORAGE, j.content.sha);
      setSyncStatus("Synced " + new Date().toLocaleTimeString());
    } else if (putRes.status === 409 && !isRetry) {
      localStorage.removeItem(GH_SHA_STORAGE);
      return pushReviewsToGitHub(true);
    } else if (putRes.status === 401 || putRes.status === 403) {
      setSyncStatus("Sync failed - token invalid or lacks permission");
    } else {
      setSyncStatus("Sync failed (" + putRes.status + ")");
    }
  } catch (e) {
...
function mergeReviews(remote) {
  const merged = Object.assign({}, reviews);
  for (const key of Object.keys(remote || {})) {
    const r = remote[key];
    const l = merged[key];
    if (!l || !l.reviewedAt || (r && r.reviewedAt && r.reviewedAt > l.reviewedAt)) {
      merged[key] = r;
    }
  }
  return merged;
...
async function fetchReviewsFromGitHub() {
  try {
    const url = `https://raw.githubusercontent.com/${GH_OWNER}/${GH_REPO}/${GH_BRANCH}/${GH_PATH}?_=${Date.now()}`;
    const res = await fetch(url, { cache: "no-store" });
    if (res.status === 404) {
      setSyncStatus(ghToken() ? "No synced state yet - this device will create it" : "No synced state on GitHub yet");
      return;
    }
    if (!res.ok) {
      setSyncStatus("Could not check GitHub for synced state (" + res.status + ")");
      return;
    }
    const remote = await res.json();
    reviews = mergeReviews(remote);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(reviews));
    renderList();
    renderProgress();
    setSyncStatus("Loaded latest from GitHub " + new Date().toLocaleTimeString());
  } catch (e) {
    setSyncStatus("Offline - using locally saved progress");
  }
}
...
document.getElementById("syncSetupBtn").addEventListener("click", () => {
  ...
  if (stored) document.getElementById("ghTokenInput").value = stored;
});
document.getElementById("toggleGhTokenVis").addEventListener("click", () => {
...
document.getElementById("saveGhTokenBtn").addEventListener("click", () => {
  ...
  if (!token) {
    document.getElementById("ghTokenError").textContent = "Paste a token first.";
    return;
  }
  localStorage.setItem(GH_TOKEN_STORAGE, token);
  localStorage.removeItem(GH_SHA_STORAGE);
  updateSyncPanelVisibility();
  document.getElementById("syncPanel").classList.remove("show");
  pushReviewsToGitHub();
});
document.getElementById("clearGhTokenBtn").addEventListener("click", () => {
  localStorage.removeItem(GH_TOKEN_STORAGE);
  localStorage.removeItem(GH_SHA_STORAGE);
  updateSyncPanelVisibility();
  document.getElementById("syncPanel").classList.remove("show");
  setSyncStatus("GitHub sync: not configured");
});

document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden" && ghPushTimer) {
    clearTimeout(ghPushTimer);
    pushReviewsToGitHub();
  }
});
